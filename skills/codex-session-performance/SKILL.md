...
Inspect successful work as well as errors. Expand composite commands to their actual stages: several different checks may each repeat the same build or setup. Compare a shared validated artifact against that repetition before recommending fewer checks. For other large phases, distinguish required content or implementation from reusable scaffolding; recommend a change only when the records support that distinction.

Estimate end-to-end savings only when supported. Label estimates and untested hypotheses; cap savings by exposed time. Recompute the next blocker, and never add overlapping opportunities. A saving above the removed interval needs evidence for additional avoided work; do not pad a measured duration into a larger, high-confidence range. Compare replacements on equivalent inputs, semantics, coverage, and cache conditions; include migration cost when material. Verify current compatibility from installed artifacts or official documentation as needed.
...
Lead with one sentence stating the main cause and best opportunity. Immediately follow with an ordered list of the highest-value actions, then a compact timeline. Do not insert a scope/background paragraph before the recommendations; include essential scope in the opening sentence or timeline. Usually include one to three recommendations; do not fill a quota. A single minor finding or no supported speedup normally needs 150–250 words; use roughly 250–450 words for a substantive single turn and 300–600 for several turns. These are editing ceilings, not targets to fill or reasons to omit decisive evidence.

- **[P0/P1/P2] Concrete action. Estimated saving: range in seconds/minutes per turn/session.** Put the action and saving on the first line so the list is useful without reading every explanation. Follow with a short paragraph: what happened, the specific tool/file/workflow change, and why it removes the delay. Include confidence, the key assumption, and a clickable source link to the decisive call/line or evidence excerpt. Mention a success check only when it adds information. Prefer about 60–90 words per finding; merge related symptoms.
- **Timeline:** use a small table with `Turn / elapsed | Sequence that determined completion | Source`. Show ordered intervals, failed/dead-end calls and their recovery, and meaningful overlapping work. Expand only the bottleneck when a detailed trace is requested; keep the complete trace in a linked local artifact when long. Use relative offsets or one stated timezone. Link source references directly; keep long IDs, ordinal reconciliation, and export bookkeeping in the linked evidence unless they change the diagnosis.
...
Put evidence and caveats next to the claims they qualify. Omit generic closing disclaimers about inference, missing telemetry, or incomplete work when no recommendation depends on them. Do not repeat each finding in separate cause, recommendation, counterfactual, and summary sections. Omit generic framing, praise, rhetorical contrasts, invented labels, and headings about the analysis process. Use concrete verbs and ordinary language. If a turn has no supported material opportunity, say so; do not invent one to fill the format.
