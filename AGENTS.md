...
- Before handoff: generate WASM (`npm run build:wasm` on a fresh checkout), run `npm run check` (TypeScript/Oxlint/oxfmt/Knip/frontend tests and Rust fmt/clippy/tests), `npm run build`, `bash scripts/test-wasm.sh`, and relevant external Chrome tests. Real-data benchmark commands are opt-in and documented in README.
